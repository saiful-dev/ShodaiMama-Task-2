
import React from "react";
import { useState, useEffect } from "react";
export default function Localstorage(props){

const object={
    id: props.id,
    title:props.title,
    price:props.price,
    
    category:props.category,
    image:props.imgSrc,
   
}

    const [product, setproduct] = useState({});

    useEffect(() => {
        // storing input name
        localStorage.setItem("product", JSON.stringify(object));
      }, [object]);

    
}




<div>
    <div class="productListArea">
        <div class="productCart ">
            <span style="cursor: pointer;">
                <div class="productContent">
                    <div class="productOrigin fresh">
                        <a href="/">Fresh</a>
                    </div>
                <div class="productContentInner">
            <div class="productImageArea">
                <img class="prodcutImg" alt="Pineapple Medium" 
                src="https://bucket.shodaimama.com/files/sm/images/smprod_1624442628390_00010391_czmIWFyz_SASSMIS.webp"/>
                </div>
                <p class="productName">
                Pineapple Medium</p>
                <p class="productSubtext">each</p>
                <p class="productPriceArea">
                <span class="quantitySymbol">৳</span>
                    <span class="prodcutPrice">45</span>
                    </p></div>
                </div></span>
                <div class="productDetailsBtnArea" style="cursor: pointer;">
                <button class="productCardDetailsBtn">Details</button>
                </div><div>
                <span class="productDetailsArea">
                <div class="productDeliveryStatus">
                <img src="/static/media/fastDelivaryIcon.86065e4e398b3a350453.png" alt="bike picture"/>
                <p>Next Morning</p></div><div class="productDeliveryText">
                <img src="data:image/png;base64," alt=""/>
                <img src="data:image/png;base64," alt="" style="margin: 0px 5px;"/>
                </div>
                </span>
                </div><div class="bottomAddToCartBtn"><button class="addToBgBtnGeneral">Add To Cart</button>
                </div></div>
                </div>
                </div>